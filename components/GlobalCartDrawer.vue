...
function inc(key) { cart.increment(key) }
function dec(key) { cart.decrement(key) }
function removeItem(key) { cart.remove(key) }
...
<template>
    <transition name="fade">
        <div v-if="isOpen" class="fixed inset-0 z-[10000]">
            ...
            <aside
                class="absolute right-0 top-0 h-full w-[92vw] max-w-md bg-white border-l border-black shadow-xl flex flex-col">
                    ...
                    <div v-for="line in lines" :key="line.key" class="flex gap-3 border border-black rounded p-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mt-2 flex items-center gap-2">
                                <button class="px-2 border border-black" @click="dec(line.key)"
                                    aria-label="Decrease">–</button>
                                <span class="text-sm w-6 text-center" aria-live="polite">{{ line.qty }}</span>
                                <button class="px-2 border border-black" @click="inc(line.key)"
                ...
                <footer class="p-4 border-t border-black space-y-2">
                    ...
                    <p class="text-[11px] text-black/60">
                        Taxes shown above. Shipping (if any) is calculated at checkout.
                    </p>
                    <div class="flex gap-2 pt-2">
                        <NuxtLink to="/cart" @click="close"
